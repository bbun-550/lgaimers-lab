"""
HTW의 EXAONE 모델 성능/속도 평가 스크립트

사용법:
    uv run python src/evaluation/evaluate_htw.py --model "경로" --experiment "htw_exp" --run-name "gptq_w4"
...
def evaluate_model(model_path: str, 
                   experiment_name: str = "htw_experiments",
                   run_name: str = None,
    ...
    with mlflow.start_run(run_name=run_name or Path(model_path).name):
        ...
        mlflow.set_tag("owner", "htw")  # 👈 HTW 태그 추가
        mlflow.set_tag("compression_type", "gptq" if "gptq" in model_path.lower() else "unknown")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="HTW's EXAONE model evaluation")
    ...
    parser.add_argument("--experiment", type=str, default="htw_experiments",
                        help="MLflow experiment name (default: htw_experiments)")
